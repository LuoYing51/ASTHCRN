...
from ASTHCRN.ablation.ASTHCRN_model_without_AdaHG import main
...
args.add_argument('--sparse_window', default=8, type=int)
args.add_argument('--edge_mask_threshold', default=0.50, type=float)
...
#     r'../ASTHCRN/data/data_distance_data.xlsx',
#     index_col=0)
# distance_matrix = distance_df.values.astype(float)
# distance_matrix_tensor=torch.from_numpy(distance_matrix).to(device)
#
# H_list, H ,hyperedge_index= build_hypergraph_distance(distance_matrix, args.KNN_num, args.distance_threshold,device)


hyperedge_index_df = pd.read_csv('../ASTHCRN/ablation/data_hyperedge_index.csv')
hyperedge_index_numpy_data = hyperedge_index_df.values
hyperedge_index = torch.from_numpy(hyperedge_index_numpy_data).to(device)
