...
class HGCRU_without_GRU(nn.Module):
    def __init__(self, in_feature, hidden_dim):
        super(HGCRU_without_GRU, self).__init__()
        self.hidden_dim = hidden_dim
        self.hgcn = HypergraphConv(in_channels=in_feature, out_channels=hidden_dim)

    def forward(self, x, state, hyperedge_index, hyperedge_weight):
        out=self.hgcn(x=x, hyperedge_index=hyperedge_index, hyperedge_weight=hyperedge_weight)
        return out
