...
class HGCRU_without_HGCN(nn.Module):
    def __init__(self, in_feature, hidden_dim):
        super(HGCRU_without_HGCN, self).__init__()
        self.hidden_dim = hidden_dim

        self.Liner1 = nn.Linear(in_feature + hidden_dim, 2 * hidden_dim)
        self.Liner2 = nn.Linear(in_feature + hidden_dim, hidden_dim)
...
class AHGCRU_without_HGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(T):
            ...
            state = self.HGCRU_without_HGCN(current_x, state, HE, HEW)
