...
class HGCRU_without_TempHG(nn.Module):
    def __init__(self, in_feature, hidden_dim):
        super(HGCRU_without_TempHG, self).__init__()
...
class AHGCRU_without_TempHG(nn.Module):
    def __init__(self, device, num_nodes, in_channels, hidden_dim,out_channels,
        ...
        self.HGCRU_without_TempHG = HGCRU_without_TempHG(in_feature=in_channels, hidden_dim=self.hidden_dim)
    ...
    def forward(self, x):
        ...
        for t in range(T):
            ...
            state = self.HGCRU_without_TempHG(current_x, state, HE, HEW)
            outputs.append(state.reshape(B, N, self.hidden_dim))

        final_output_ = torch.stack(outputs, dim=-1)  # 形状: [B, N, C_out, T]
