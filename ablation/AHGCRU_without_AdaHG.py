...
class HyperGCNBlock_without_AdaHG(nn.Module):
    def __init__(self, in_c, out_c, hidden_c=None):
        super(HyperGCNBlock_without_AdaHG, self).__init__()
...
class DeepHyperGCN_GRUCell_without_AdaHG(nn.Module):
    def __init__(self, in_feature, hidden_dim):
        super(DeepHyperGCN_GRUCell_without_AdaHG, self).__init__()
        self.hidden_dim = hidden_dim
        self.gate_block = HyperGCNBlock_without_AdaHG(in_c=in_feature + hidden_dim, out_c=2 * hidden_dim)

        self.candidate_block = HyperGCNBlock_without_AdaHG(in_c=in_feature + hidden_dim, out_c=hidden_dim)

    def forward(self, x, state, hyperedge_index):
        input_and_state = torch.cat((x, state), dim=1)

        z_r = self.gate_block(input_and_state, hyperedge_index)
        z_r = torch.sigmoid(z_r)
        z, r = torch.split(z_r, self.hidden_dim, dim=1)

        candidate_input = torch.cat((x, r * state), dim=1)

        hc = self.candidate_block(candidate_input, hyperedge_index)
        hc = torch.tanh(hc)
...
class AHGCRU_without_AdaHG(nn.Module):
    def __init__(self, device, num_nodes, in_channels, hidden_dim,out_channels,
                 hyperedge_index, hyperedge_rate, HGCNADP_topk, embed_dims):
        super(AHGCRU_without_AdaHG, self).__init__()
        self.device = device
        ...
        self.hgru_cell = DeepHyperGCN_GRUCell_without_AdaHG(in_feature=in_channels, hidden_dim=self.hidden_dim)
    ...
    def forward(self, x,):
        ...
        for t in range(T):
            ...
            state = self.hgru_cell(current_x, state, self.hyperedge_index)
