...
from components.ASTHCRN_model import main
...
logger.info("Calculating and saving metrics per timestep for each substation/load zone...")
num_timesteps = denorm_pred.shape[1]

metrics_by_timestep = {
    'MAE': np.zeros((num_timesteps, args.num_nodes)),
    'RMSE': np.zeros((num_timesteps, args.num_nodes)),
    'MAPE': np.zeros((num_timesteps, args.num_nodes)),
    'MSE': np.zeros((num_timesteps, args.num_nodes)), }

for t in range(num_timesteps):
    for n in range(args.num_nodes):
        pred_tn = denorm_pred[:, t, n]
        true_tn = denorm_true[:, t, n]

        mae_tn, rmse_tn, mape_tn, mse_tn, _ = All_Metrics(pred_tn, true_tn)

        metrics_by_timestep['MAE'][t, n] = mae_tn.item()
        metrics_by_timestep['RMSE'][t, n] = rmse_tn.item()
        metrics_by_timestep['MAPE'][t, n] = mape_tn.item()
        metrics_by_timestep['MSE'][t, n] = mse_tn.item()

station_columns = [f'Station_{i + 1}' for i in range(args.num_nodes)]

for metric_name, data_matrix in metrics_by_timestep.items():
    if not np.any(data_matrix):
        continue

    df = pd.DataFrame(data_matrix, columns=[f'{col}_{metric_name}' for col in station_columns])
    df.insert(0, 'Horizon', range(1, num_timesteps + 1))

    csv_filename = f'{metric_name.lower()}_per_timestep.csv'
    csv_save_path = os.path.join(args.log_dir, name + csv_filename)
    df.to_csv(csv_save_path, index=False, float_format='%.4f')
    logger.info(f"Saved {metric_name} per timestep to: {csv_save_path}")
